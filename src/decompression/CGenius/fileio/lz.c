...
char lz_decompress(FILE *lzfile, unsigned char *outbuffer)
{
	...
	lzdict = (stLZDictionaryEntry **)malloc(maxdictsize*sizeof(stLZDictionaryEntry *));
	...
	for(i=0;i<maxdictsize;i++)
	{
		...
		if (!lzdict[i])
		{
			fprintf(stderr, "lz_decompress(): unable to allocate memory for dictionary!\n");
			return 1;
		}
	}
	...
	do
	{
		...
		if (addtodict)     // room to add more entries to the dictionary?
		{
			...
			{
				fprintf(stderr, "lz_decompress(): lzdict[%d]->stringlen is too long...max length is %d\n", dictindex, LZ_MAXSTRINGSIZE);
				return 1;
			}
